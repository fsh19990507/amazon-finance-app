...
import {
  DollarOutlined, RiseOutlined, PercentageOutlined, FundOutlined, DeleteOutlined
...
export default function AdvertisingAnalysis() {
  ...
  const handleDelete = async (record) => {
    ...
    try {
      await db.adReports.delete(record.id);
  ...
  // 加载广告报告数据（按当前选中店铺过滤；「全部店铺」时不限制）
  const data = useLiveQuery(
    () => db.adReports.toArray().then((rows) =>
