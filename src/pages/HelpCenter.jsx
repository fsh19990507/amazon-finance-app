        ...
        dataSource={[
          {
            ...
            color: 'orange',
            title: '离线模式说明',
            description: '云端连不上时系统自动进入离线模式：数据只读（不能新增/删除/编辑），本地缓存的历史报表仍可正常查看；云端恢复后数据会自动重新同步。'
          },
          {
            tag: '汇率',
            color: 'blue',
        ...
        items={[
          {
            ...
            children: (
              <div>
                <p><Text strong>人话解答：</Text>系统数据存在云端服务器，网络不通时会自动改用你本机缓存的旧数据，期间功能暂时受限，但数据不会丢。</p>
                <ul style={{ margin: 0, paddingLeft: 20 }}>
                  <li>检查网络：确认能正常上网（可以试着打开其他网站）</li>
                  <li>检查代理 / VPN：国内网络访问 supabase.co 可能被限制，可切换网络或开启代理后重试</li>
                  <li>稍等重试：云端检测约 5 秒超时，网络恢复后会自动重新连接</li>
                  <li>数据安全：离线期间的数据，云端恢复后会自动同步</li>
                </ul>
              </div>
            )
          },
